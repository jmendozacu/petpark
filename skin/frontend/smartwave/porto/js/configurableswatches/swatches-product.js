        ...
        if (opt) {
            if (opt != attr._e._last.selectedOption) {
                ...
                if (opt._f.isSwatch) {
                    ...
                    /*jQuery('.configurable-swatch-list .selected').each(function(){
                      
                    });*/
                          var def = 'o';
                    /*jQuery('.super-attribute-select').each(function(){
                      def = def + '_'+jQuery(this).val();
                    });
                    var def2 = 'o';*/
                    jQuery('.configurable-swatch-list .selected').each(function(){
                      def = def + '_'+ jQuery(this).attr('id');
                    });
                    
                    //neco nezaskrtnute
                    //if (jQuery('.super-attribute-select:not(:selected)').length){
                    if (jQuery('.configurable-swatch-list:not(:has(.selected))').length){
                        jQuery('.availability span').text(Translator.translate('Please select all parameters.'));
                    }else{
                    
                        //console.log('checkuju2 '+def2);//koh
                        console.log('checkuju '+def);//koh
                        if (availabilityInfo[def]){
                          console.log(availabilityInfo[def]);
                          jQuery('.availability span').text(availabilityInfo[def]); 
                        }else{
                          console.log('neni');
                          jQuery('.availability span').text(Translator.translate('Out of Stock'));
                        }//koh
                    }
//                    configurable-swatch-list
    ...
    checkStockStatus: function() {
        ...
        checkOptions.each( function(selectedOpt) {
            //console.log(selectedOpt.products);//koh
