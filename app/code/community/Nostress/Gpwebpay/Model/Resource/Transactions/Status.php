...
/**
 * Transaction status resource model
 *
 * @category   Nostress
 * @package    Nostress_Gpwebpay
 * @author     NoStress Commerce Team <info@nostresscommerce.cz>
 */
class Nostress_Gpwebpay_Model_Resource_Transactions_Status extends Mage_Core_Model_Resource_Db_Abstract
{
	/**
	* Status labels table
	*
	...
	*/
	protected function _construct() {
		$this->_init('nostress_gpwebpay/transactions_status', 'status');
		$this->_isPkAutoIncrement = false;
		$this->_labelsTable = $this->getTable('nostress_gpwebpay/transactions_status_label');
		$this->_stateTable  = $this->getTable('nostress_gpwebpay/transactions_status_state');
	}
    ...
    public function unassignState($status, $state)
    {
        ...
        if ($this->_getWriteAdapter()->fetchOne($select) == 1) {
            throw new Mage_Core_Exception(
                Mage::helper('nostress_gpwebpay')->__('Last status can not be unassigned from state.')
            );
        }
        $select = $this->_getWriteAdapter()->select()
