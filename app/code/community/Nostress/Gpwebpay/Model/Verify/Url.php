 ...
 */
class Nostress_Gpwebpay_Model_Verify_Url extends Mage_Core_Model_Config_Data
{
	protected function _beforeSave() {
		$value = $this->getValue();
		
		if (!empty($value)) {
			if (!preg_match("/^https:\/\/+[A-Za-z0-9\-_]+\\.+[A-Za-z0-9\.\/%&=\?\-_]+$/i", $value))
				Mage::throwException("Error: URL is not valid!");
		}
