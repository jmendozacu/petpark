...
class Flagbit_FilterUrls_Model_Catalog_Layer_Filter_Item extends Mage_Catalog_Model_Layer_Filter_Item
{
     ...
     */
    public function getSpeakingFilterUrl($addOwnValue, $withoutFilter = FALSE, $additionalQueryParams = array(), $withoutExcludedParams = false)
    {
        $filterUrlArray = $this->_getFilterUrlArrayForCurrentState($withoutFilter, $withoutExcludedParams);
     ...
     */
    protected function _getFilterUrlArrayForCurrentState($withoutFilter, $withoutExcludedParams)
    {
        $filterUrlArray = array();
        $attributesArray = array();
        $query = array();
        $seoAttributesHelper = null;
        if ($withoutExcludedParams) {
        ...
        foreach (Mage::getSingleton('catalog/layer')->getState()->getFilters() as $item) {
            ...
            if ($seoAttributesHelper && in_array($item->getFilter()->getAttributeModel()->getAttributeCode(), $seoAttributesHelper->getExcludedAttributes())) {
                continue;
            }
