...
class Smartwave_Megamenu_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	$catCacheFile = 'cat-'.Mage::app()->getStore()->getCode().'.cache';
	if(file_exists($catCacheFile)) {
		$cache = unserialize(file_get_contents($catCacheFile));
		if($cache['time'] + 900 > time()) {
	...
	file_put_contents($catCacheFile, serialize(['time' => time(), 'cats'=>$categories]));
