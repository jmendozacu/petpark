...
class Importer{
    ...
    public function parseMain($out){
        ...
        $col = Mage::getModel('catalog/product')->getCollection()->addAttributeToSelect('availability_out');
        ...
        foreach($col as $p){
          ...
          $availabilities[$p->getSku()] = $p->getAttributeText('availability_out');
        }

        
        $csv = new Zebu_Tools_CSV_AssocWriter($out,
                array(
                    'sku', 'ean', 'qty', 'in_in_stock', 'availability'                 
                )
                ,',');
        ...
        foreach($responseArray['products'] as $data){
            if ($data['warehouseCode'] != 'SC'){
             ...
             $row = array(
               ...
               'availability' => ($qty>0) ? 'skladom' : $availabilities[$sku]
             );
